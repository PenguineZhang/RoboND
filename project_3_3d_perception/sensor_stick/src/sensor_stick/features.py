...
def compute_color_histograms(cloud, using_hsv=False):
    ...
    channel123 = np.concatenate((channel_1_hist[0], channel_2_hist[0], channel_3_hist[0])).astype(np.float64)

    normed_features = channel123 / np.sum(channel123)
...
def compute_normal_histograms(normal_cloud):
    ...
    # TODO: Concatenate and normalize the histograms
    norm_xyz = np.concatenate((norm_x_hist[0], norm_y_hist[0], norm_z_hist[0])).astype(np.float64)

    normed_features = norm_xyz / np.sum(norm_xyz)
    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
#    normed_features = np.random.random(96)
